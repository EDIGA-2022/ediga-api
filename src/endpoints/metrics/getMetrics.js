...
async function getMetrics (req, res) {
  ...
  const userGenders = await getGenders()

  // Get ages and amount for each age
  const userAges = await getAges()
  ...
  const middleFormAnswersData = await getMiddleFormAnswers(totalUsers)
  const endFormAnswersData = await getEndFormAnswers(totalUsers)
...
async function getGenders () {
  ...
  userGendersQuery[0].forEach(item => {
    const currentItem = {
...
async function getAges () {
  const ages = await UserRegisterInfo.findAll({
...
async function getMiddleFormAnswers (totalUsers) {
  // Middle form answers
  const middleFormAnswer1 = await MiddleFormAnswer.findAll({
  ...
  const middleFormAnswer2 = await MiddleFormAnswer.findAll({
    attributes: [['Answer_2', 'answer'], [db.sequelize.fn('COUNT', db.sequelize.col('Answer_2')), 'amount']],
    group: ['Answer_2']
  })

  const answersSecondQuestion = []
  mostPopularAnswer = ''
  mostPopularAmount = 0
...
async function getEndFormAnswers (totalUsers) {
  // Middle form answers
  const endFormAnswer1 = await EndFormAnswer.findAll({
    attributes: [['Answer_1', 'answer'], [db.sequelize.fn('COUNT', db.sequelize.col('Answer_1')), 'amount']],
    group: ['Answer_1']
  })
  ...
  const endFormAnswers = []
  const endFormAnswer1Object = {
  ...
  const endFormAnswer2 = await EndFormAnswer.findAll({
    attributes: [['Answer_2', 'answer'], [db.sequelize.fn('COUNT', db.sequelize.col('Answer_2')), 'amount']],
    group: ['Answer_2']
  })
